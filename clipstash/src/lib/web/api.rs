...
pub enum ApiKeyError {
    #[error("API key not found")] #[response(status = 404, content_type = "json")] NotFound(String),
    #[error("invalid API key format")] #[response(status = 400, content_type = "json")] DecodeError(
        String,
    ),
}
...
pub enum ApiError {
    #[error("not found")] #[response(status = 404, content_type = "json")] NotFound(Json<String>),
    #[error("server not found")] #[response(status = 500, content_type = "json")] Server(
        Json<String>,
    ),
    #[error("client not found")] #[response(status = 401, content_type = "json")] User(
        Json<String>,
    ),
    #[error("key not found")] #[response(status = 400, content_type = "json")] KeyError(
        Json<ApiKeyError>,
    ),
}
