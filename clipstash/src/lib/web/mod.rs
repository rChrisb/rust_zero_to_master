...
pub enum PageError {
    #[response(status = 500)] Serialization(String),
    #[response(status = 500)] Render(String),
    #[response(status = 500)] NotFound(String),
...
pub mod test {
    ...
    pub fn config() -> RocketConfig {
        ...
        let hit_count = HitCounter::new(database.get_pool().clone(), rt.handle.clone());
