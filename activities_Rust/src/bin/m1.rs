...
mod test {
    ...
    fn errors_when_user_not_logged_in() {
        ...
        let user = UserId(1);
        let data = get_data(resource, user);
        assert!(matches!(data, Err(ServeError::NotLoggedIn)), "should be a NotLoggedIn error");
    }
    ...
    fn errors_when_user_not_authorized() {
        ...
        let user = UserId(2);
        let data = get_data(resource, user);
        assert!(matches!(data, Err(ServeError::Unauthorized)), "should be an Unauthorized error");
    }
